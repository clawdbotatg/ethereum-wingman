...
# - Production URL (name.yourname.eth.link)
...
NEXT_PUBLIC_PRODUCTION_URL="https://<name>.yourname.eth.link" \
...
1. Open `https://app.ens.domains/yourname.eth` in the wallet browser (your MetaMask browser)
...
curl -s -o /dev/null -w "%{http_code}" -L "https://<name>.yourname.eth.link"

# 3. OG metadata correct
curl -s -L "https://<name>.yourname.eth.link" | grep 'og:image'
